...
import { Calendar as CalendarIcon, X, Trash2, Pencil, Check } from "lucide-react";
...
interface TaskDetailDialogProps {
  ...
  onUpdateLog: (taskId: string, logId: string, logContent: string) => void;
  /** Callback to delete a work log. */
...
export function TaskDetailDialog({
  ...
  const handleUpdateLog = () => {
    if (editingLog && editingLog.content.trim()) {
      onUpdateLog(task.id, editingLog.id, editingLog.content.trim());
      setEditingLog(null);
    }
  };
  ...
  const handleDeleteLog = () => {
    if (logToDelete) {
      onDeleteLog(task.id, logToDelete.id);
      setLogToDelete(null);
    }
  };
                    ...
                    {editingLog?.id === log.id ? (
                       <div className="flex items-start gap-2">
                         ...
                         <div className="flex flex-col gap-1">
                           <Button size="icon" className="h-7 w-7" onClick={handleUpdateLog}><Check className="h-4 w-4" /></Button>
                           <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setEditingLog(null)}><X className="h-4 w-4" /></Button>
                         </div>
                       </div>
                    ) : (
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                           <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => setEditingLog({ id: log.id, content: log.content })}>
                             <Pencil className="h-4 w-4" />
                           </Button>
                           <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => setLogToDelete(log)}>
